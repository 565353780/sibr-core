...
#include <projects/remote_v2/renderer/RemotePointView.hpp>
#include <thread>
...
void sibr::RemotePointView::set_render_items(
    boost::asio::ip::tcp::socket &sock) {
  uint32_t data_length;
  boost::system::error_code ec;
  // Read the length of the serialized data
  boost::asio::read(sock,
                    boost::asio::buffer(&data_length, sizeof(data_length)), ec);
  if (ec) {
  ...
  json deserialized_data =
      json::parse(serialized_data.begin(), serialized_data.end());
  std::vector<std::string> string_list =
      deserialized_data.get<std::vector<std::string>>();

  //  include an additional item in _renderItems vector that corresponds to the
  //  "Show Input Points" option
      ...
      while (keep_running) {
        {
          ...
          // Serialize our arbitrary data to something simple, yet convenient
          // for both sides
          json sendData;
          ...
          sendData[jViewMat] = std::vector<float>(
              (float *)&_remoteInfo.view, ((float *)&_remoteInfo.view) + 16);
          sendData[jViewProjMat] =
              std::vector<float>((float *)&_remoteInfo.viewProj,
          ...
          boost::asio::write(
              sock, boost::asio::buffer(&messageLength, sizeof(uint32_t)));
          boost::asio::write(
              sock, boost::asio::buffer(message.c_str(), messageLength));
        }

        uint32_t bytes_to_receive =
            _remoteInfo.imgResolution.x() * _remoteInfo.imgResolution.y() * 3;
        if (bytes_to_receive > 0) {
          std::lock_guard<std::mutex> ilg(_imageDataMutex);
          _imageData.resize(bytes_to_receive);
          boost::asio::read(
              sock, boost::asio::buffer(_imageData.data(), _imageData.size()));
          {
            std::lock_guard<std::mutex> lg(_renderDataMutex);
        ...
        boost::asio::read(sock,
                          boost::asio::buffer(&sceneLength, sizeof(uint32_t)));
        std::vector<char> sceneName(sceneLength);
        boost::asio::read(sock,
                          boost::asio::buffer(sceneName.data(), sceneLength));
        ...
        // Read the length of the serialized data
        boost::asio::read(
            sock, boost::asio::buffer(&data_length, sizeof(data_length)), ec);
        if (ec) {
          throw boost::system::system_error(
              ec); // Or handle error as appropriate
        }
        // Read the serialized data
        std::vector<char> serialized_data(data_length);
        boost::asio::read(sock, boost::asio::buffer(serialized_data), ec);
        ...
        json deserialized_data =
            json::parse(serialized_data.begin(), serialized_data.end());
        globalMetricsDict =
            deserialized_data.get<std::map<std::string, double>>();
  ...
  if (ImGui::Begin(guiName.c_str())) {
    ...
    for (int i = 0; i < _renderItems.size(); ++i) {
      if (ImGui::RadioButton(_renderItems[i].c_str(), &_item_current, i)) {
        // When the "Show Input Points" option is selected, set _showSfM
        // accordingly
