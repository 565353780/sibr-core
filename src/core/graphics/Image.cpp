...
sibr::ImageRGB32F
convertSphericalHalfToNormalMap(const sibr::ImageRGBA &imgRGBA) {
  uint phi_uint;
  uint theta_uint;
  unsigned char *phi_ptr = reinterpret_cast<unsigned char *>(&phi_uint);
  unsigned char *theta_ptr = reinterpret_cast<unsigned char *>(&theta_uint);
...
coloredClass(const Image<unsigned char, 1>::Ptr imClass) {
...
void showFloat(const Image<float, 1> &im, bool logScale, double min,
               double max) {
    ...
    for (unsigned int i = 0; i < im.w(); i++) {
      if (logScale)
        imIntensity(i, j).x() = static_cast<unsigned char>(std::max(
            0.0,
            std::min((log(im(i, j).x()) - min) * 255 / (max - min), 255.0)));
      else
        imIntensity(i, j).x() = static_cast<unsigned char>(std::max(
            0.0, std::min((im(i, j).x() - min) * 255 / (max - min), 255.0)));
    }
  }
  cv::Mat colorMat;
...
sibr::Vector2i IImage::imageResolution(const std::string &file_path) {
  ...
  case PNG:
    ...
    width = sibr::ByteStream::htonl(width);
    height = sibr::ByteStream::htonl(height);
    break;
  case BMP:
    file.seekg(14);
