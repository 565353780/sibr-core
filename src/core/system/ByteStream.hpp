...
namespace sibr {
/// Be sure to use STL objects from client's dll version by exporting this
/// declaration (see warning C4251)
// template class SIBR_EXPORT std::vector<uint8>;

/**
 Used to manipulate stream of bytes.
 \note This ByteStream stores integer using the network byte order (which is big
 endian). \ingroup sibr_system
*/
...
public:
  ...
  // We don't want to include network-related libs (and all their stuffs), so we
  // use a custom implementation of htonl/htons, htonl/htons.
...
///// INLINE FUNCTIONS /////
size_t ByteStream::readableSize(void) const { return bufferSize() - _readPos; }
size_t ByteStream::bufferSize(void) const { return _buffer.size(); }
...
ByteStream &ByteStream::operator>>(int16 &i) {
  if (testSize(sizeof(i))) {
    i = htons(*reinterpret_cast<int16 *>(&_buffer[_readPos]));
    _readPos += sizeof(i);
  }
  return *this;
}
...
ByteStream &ByteStream::operator>>(int32 &i) {
  if (testSize(sizeof(i))) {
    i = htonl(*reinterpret_cast<int32 *>(&_buffer[_readPos]));
    _readPos += sizeof(i);
  }
  return *this;
}
ByteStream &ByteStream::operator>>(int64 &i) {
  if (testSize(sizeof(i))) {
    i = htonll(*reinterpret_cast<int64 *>(&_buffer[_readPos]));
    _readPos += sizeof(i);
  }
  return *this;
}
...
ByteStream &ByteStream::operator>>(uint16 &i) {
  if (testSize(sizeof(i))) {
    i = htons(*reinterpret_cast<uint16 *>(&_buffer[_readPos]));
    _readPos += sizeof(i);
  }
  return *this;
}
ByteStream &ByteStream::operator>>(uint32 &i) {
  if (testSize(sizeof(i))) {
    i = htonl(*reinterpret_cast<uint32 *>(&_buffer[_readPos]));
    _readPos += sizeof(i);
  }
  return *this;
}
ByteStream &ByteStream::operator>>(uint64 &i) {
  if (testSize(sizeof(i))) {
    i = htonll(*reinterpret_cast<uint64 *>(&_buffer[_readPos]));
    _readPos += sizeof(i);
  }
  return *this;
}
...
// Function used to test this class (might be still useful to test future
// improvement)
/*
static void unitTestByteStream( void )
{
        ...
        bytes << di.b << di.s8 << di.u8 << di.s16 << di.u16 << di.s32 << di.u32
<< di.f; SomeData dout = {false, 0, 0, 0, 0, 0, 0, 0.f}; std::cout << "Dumping
DataOUT:" << std::endl; dout.dump();

        bytes.memoryDump();


        bytes >> dout.b >> dout.s8 >> dout.u8 >> dout.s16 >> dout.u16 >>
dout.s32 >> dout.u32 >> dout.f; std::cout << "Dumping DataOUT:" << std::endl;
        dout.dump();
}
*/
